...
public class Class2 {
	...
	public static void main(String[] args) {
		...
		Cat catTwo = new Cat();
		catTwo.name = "Tom";
		// catOne.name = null; //catOne下面還有做處理 定義null會出錯
		System.out.println("catOne.name.equals(catTwo.name) => " + catOne.name.equals(catTwo.name)); // 物件儲存的內容 是否一致。
		System.out.println("catOne.name == catTwo.name  	=> " + (catOne.name == catTwo.name)); // 物件的資料型態，物件的記憶体位址(id=?)。
		System.out.println("catOne.equals(catTwo)       	=> " + catOne.equals(catTwo)); // 內容的物件是否一致。
		System.out.println("catOne == catTwo            	=> " + (catOne == catTwo)); // ==指物件的記憶体位址 是否一致。
...
class BasicType {
	...
	public int s1() {
		iAmInt = 10;
		return 10 - iAmInt * iAmInt - iAmInt + iAmInt / iAmInt;
